...
function PodiumSpot({
  ...
  const tribe = player.badge,
    isFirst = rank === 0;
  return <div className="flex flex-1 flex-col items-center"><div className={`${isFirst ? "text-[28px]" : "text-[22px]"} mb-1.5 leading-none`}>{MEDALS[rank]}</div><div className={`relative flex items-center justify-center rounded-full font-serif font-black ${isFirst ? "h-[68px] w-[68px] text-lg" : "h-14 w-14 text-[15px]"}`} style={{
      ...
      borderBottom: "none"
    }}><span className="font-serif text-xl font-black" style={{
...
function LeaderboardRow({
  ...
  }}><span className="w-7 shrink-0 text-center font-serif text-base font-black" style={{
      color: isCurrentUser ? myBadge.color : "var(--color-muted)"
    }}>{rank}</span><MinusIcon width={13} height={13} className="text-muted" /><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full font-serif text-[13px] font-black" style={{
      background: withOpacity(tribe.color, 0.1),
...
export default function Leaderboard() {
  ...
  return <div className={`space-y-4 ${currentRank > 3 ? "pb-24" : "pb-4"}`}><div><p className="eyebrow">Leaderboards</p><h1 className="font-serif text-xl font-bold tracking-tight">Who Leads the Pack? 🏆</h1></div><div className="flex items-end gap-1.5">{topThree.map((player, index) => {
      ...
      }}><span className="w-7 text-center font-serif text-lg font-black" style={{
          color: currentBadge.color
        }}>#{currentRank}</span><MinusIcon width={13} height={13} className="text-muted" /><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full font-serif text-[13px] font-black" style={{
