...
export default function ConquestWeek() {
  ...
  const items = (week == null ? void 0 : week.items) ?? [],
    weekItems = items.filter(item => days.includes(item.date)),
    selectedDate = days[selectedDayIndex],
    dayItems = weekItems.filter(item => item.date === selectedDate),
    doneCount = weekItems.filter(item => item.done).length,
